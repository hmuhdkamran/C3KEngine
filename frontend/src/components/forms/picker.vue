<template>
    <div ref="vTailwindPickerRef" class="relative select-none" v-closable="{
        handler: 'onAway',
        exclude: ['currentPicker'],
    }" @click="onFeedBack" :style="`--bg-tailwind-picker: ${theme.background};`">
        <slot></slot>
        <transition name="v-tailwind-picker">
            <div v-show="showPicker || inline">
                <div id="v-tailwind-picker" class="bg-transparent mt-3 z-10" :class="[
                    { 'inline-mode': inline },
                    inline ? 'static' : 'absolute bottom-0 inset-x-0',
                    theme.currentColor,
                ]">
                    <div class="w-88 h-auto max-w-xs transition-all duration-150 ease-linear rounded overflow-hidden border"
                        :class="[
                            theme.border,
                            theme.text,
                        ...
                        <div class="relative p-1">
                            ...
                            <div class="flex justify-between items-center relative">
                                <div class="flex-shrink-0 w-8">
                                    <transition name="v-tailwind-picker-chevron-left">
                                        <div v-if="!enableMonth && !enableYear" class="rounded-full overflow-hidden">
                                            <div class="transition duration-150 ease-out p-2" :class="[
                                                visiblePrev
                                                    ? 'cursor-pointer'
                                                    : 'cursor-not-allowed opacity-30',
                                                theme.navigation.hover,
                                            ]" @click="onPrevious()">
                                                <svg class="h-4 w-auto" xmlns="http://www.w3.org/2000/svg"
                                                    viewBox="0 0 511.641 511.641" fill="currentColor">
                                                    <path
                                ...
                                <div class="flex flex-1">
                                    <div class="flex-1 rounded overflow-hidden py-1 ml-2 mr-1 text-center cursor-pointer transition duration-150 ease-out"
                                        :class="[
                                    ...
                                    <div class="flex-1 rounded overflow-hidden py-1 mr-2 ml-1 text-center cursor-pointer transition duration-150 ease-out"
                                        :class="[
                                    ...
                                    <transition name="v-tailwind-picker-chevron-right">
                                        <div v-if="!enableMonth && !enableYear" class="rounded-full overflow-hidden">
                                            <div class="transition duration-150 ease-out p-2" :class="[
                                                ...
                                                <svg class="h-4 w-auto" xmlns="http://www.w3.org/2000/svg"
                                                    viewBox="0 0 511.949 511.949" fill="currentColor">
                                                    <path
                        ...
                        <div class="smooth-scrolling overflow-x-hidden overflow-y-auto">
                            <transition name="v-tailwind-picker-body">
                                <div v-if="!enableMonth && !enableYear" class="relative">
                                    <div class="flex flex-nowrap py-2 border-b" :class="theme.border">
                                        <div v-for="day in days" :key="day" class="w-1/7 flex justify-center">
                                            <div class="leading-relaxed text-sm" :class="[
                                                day === 'Sun'
                                    ...
                                    <div ref="currentPicker" class="flex flex-wrap relative">
                                        <div v-for="(date, i) in previousPicker"
                                            :key="`${date.$D}${date.$M}${date.$y}-previous`"
                                            class="w-1/7 flex justify-center my-2px cursor-not-allowed" :class="[
                                            ...
                                            <div class="h-8 w-8 flex justify-center items-center"
                                                ...
                                                <div class="text-xs opacity-75" :class="[
                                                    date.day() === 0
                                                        ? theme.picker.holiday
                                                        : date.day() === 5
                                                            ? theme.picker.weekend
                                        ...
                                        <div v-for="date in currentPicker"
                                            ...
                                            <div class="relative overflow-hidden" :class="theme.picker.rounded">
                                                <div v-if="date.$events" class="absolute top-0 right-0 h-2 w-2 z-2"
                                                    :class="theme.picker.event" :style="{
                                                        backgroundColor: date.$events.color
                                                            ? date.$events.color
                                                            : '',
                                                    }"></div>
                                                <div class="relative h-8 w-8 flex justify-center items-center overflow-hidden"
                                                    :class="[
                                        ...
                                        <div v-for="date in nextPicker" :key="`${date.$D}${date.$M}${date.$y}-next`"
                                            class="w-1/7 flex justify-center my-2px cursor-not-allowed" :class="[
                        ...
                        " id="v-tailwind-picker-footer">
                            <transition-group name="v-tailwind-picker-footer" tag="div"
                                class="flex flex-wrap border-t p-1" :class="theme.event.border">
                                <div v-for="(event, i) in currentPicker.filter(
                                    (o) => o.$events !== undefined,
                                )" :key="`${i}-event`" class="w-full flex flex-row space-x-1 mb-px">
