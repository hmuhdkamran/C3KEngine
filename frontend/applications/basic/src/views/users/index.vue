...
const saveUser = () => {
  if (!isEditMode.value && entity.value.Password !== confirmPassword.value) {
    alert('Passwords do not match');
    return;
  }

  if (isEditMode.value) {
    service.update(entity.value).then(() => {
      alert('User updated successfully');
      fetchUsers();
      closeForm();
    });
  } else {
    service.add(entity.value).then(() => {
...
const deleteUser = (userId: string) => {
  if (confirm('Are you sure you want to delete this user?')) {
    service.delete(userId).then(() => fetchUsers());
  }
};
...
const toggleStatus = () => {
  entity.value.StatusId = entity.value.StatusId === 'Active' ? 'Inactive' : 'Active';
};

</script>

<template>
  <div class="user-management p-6 relative">
    <div v-if="isFormOpen" class="p-4 bg-white rounded-sm shadow-sm">
      ...
      <form @submit.prevent="saveUser" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="mb-3" v-if="!isEditMode">
          ...
          <select id="UserRole" v-model="entity.UserRole" required
            class="mt-1 block w-full p-1 rounded-sm shadow-sm focus:ring-[var(--ring-color)] focus:border-[var(--border-color)]"
            :style="{ '--ring-color': selectColor(), '--border-color': selectColor() }">
