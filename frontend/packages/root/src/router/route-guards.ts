...
import { TokenHelper } from '@/plugins/token-helper'
import type { IUser } from '@/plugins/models'
import { can } from '@/plugins/casl/casl'
import { IClaimMeta } from "./route-meta";
...
export const routeCheck = (user: IUser, to: string): boolean => {
  ...
  return user.authenticated && requiredPermissions.every(_permission => can('read', to))
};
