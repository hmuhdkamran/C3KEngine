...
<template>
  <div>
    <div v-if="props.isVisible" class="fixed inset-0 bg-black bg-opacity-30 w-full h-full z-[998]" @click="onClose">
    </div>
    <transition name="slide">
      <div v-show="props.isVisible"
        class="bg-white w-82 h-full fixed right-0 top-0 shadow-lg overflow-hidden flex flex-col z-[999]">
        ...
        <div class="p-4 overflow-y-auto flex-grow">
          <div v-if="props.isEdit">
            ...
            <input v-model="formData.title" placeholder="Enter Title" class="w-full px-3 py-1 mb-4 input-complete"
              required />
            <span class="font-semibold text-gray-700">Description:</span>
            <textarea v-model="formData.description" placeholder="Enter Description"
              class="w-full px-3 py-1 mb-4 input-complete" required></textarea>
          </div>
          <div v-else>
            <span class="font-semibold text-gray-700">Title:</span>
            <input v-model="formData.title" placeholder="Enter Title" class="w-full px-3 py-1 mb-4 input-complete"
            ...
            <span class="font-semibold text-gray-700">Description:</span>
            <textarea v-model="formData.description" placeholder="Enter Description"
              class="w-full px-3 py-1 mb-4 input-complete" disabled></textarea>
          </div>
          <div class="mb-4 relative">
            <label class="block text-gray-700 font-semibold mb-2" for="status">Status:</label>
            <div v-if="props.isEdit" class="relative">
              <select v-model="formData.status" id="status" class="w-full input-complete px-3 py-1"
