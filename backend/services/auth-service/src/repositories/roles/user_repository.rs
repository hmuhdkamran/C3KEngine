use crate::models::roles::user::User;
use c3k_common::{
...
impl IRepository<User> for UserRepository {
    async fn get_all(connection: PgPool) -> Result<Vec<User>, Box<dyn StdError>> {
        let result = sqlx::query(format!("SELECT {} FROM {}", User::COLUMNS, User::TABLE).as_str())
    ...
    async fn get_by_filter(
        connection: PgPool,
        filter: &String,
    ) -> Result<Vec<User>, Box<dyn StdError>> {
        let query = format!(
            r#"SELECT {} FROM {} WHERE {}"#,
            User::COLUMNS,
            User::TABLE,
