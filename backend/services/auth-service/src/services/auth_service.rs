...
impl AuthService {
    ...
    fn generate_jwt(&self, user: &User, claims: &Vec<Auth>) -> Result<String, Box<dyn StdError>> {
    ...
    async fn validate(
        ...
        let entities = match UserRepository::get_by_filter(self.db_pool.clone(), &user_query).await
        {
