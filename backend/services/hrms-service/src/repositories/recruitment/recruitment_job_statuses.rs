...
impl IRepository<RecruitmentJobStatuses> for RecruitmentJobStatusesRepository {
    async fn get_all(connection: PgPool) -> Result<Vec<RecruitmentJobStatuses>, Box<dyn StdError>> {
        let result = sqlx::query(format!("SELECT {} FROM {}", RecruitmentJobStatuses::COLUMNS, RecruitmentJobStatuses::TABLE).as_str())
    ...
    async fn get_by_filter(
        ...
        let query = format!(
            r#"SELECT {} FROM {} WHERE {}"#,
            RecruitmentJobStatuses::COLUMNS,
...
let _ = args.add(entity.status_id.clone());
        ...
        sqlx::query_with(
            format!("UPDATE {} SET {}", RecruitmentJobStatuses::TABLE, RecruitmentJobStatuses::COLUMNS_UPDATE).as_str(),
            args,
        )
        .execute(&connection)
        .await
    ...
    async fn delete(connection: PgPool, id: &String) -> Result<bool, Box<dyn StdError>> {
        ...
        sqlx::query_with(
            format!("DELETE FROM {} WHERE {}", RecruitmentJobStatuses::TABLE, RecruitmentJobStatuses::PK).as_str(),
            args,
        )
        .execute(&connection)
        .await
