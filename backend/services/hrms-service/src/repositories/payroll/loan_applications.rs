...
impl IRepository<LoanApplications> for LoanApplicationsRepository {
    async fn get_all(connection: PgPool) -> Result<Vec<LoanApplications>, Box<dyn StdError>> {
        let result = sqlx::query(&LoanApplications::build_select_string(
            LoanApplications::TABLE,
            &LoanApplications::COLUMNS_ARRAY,
            None,
        ))
    ...
    async fn add(connection: PgPool, entity: &LoanApplications) -> Result<bool, Box<dyn StdError>> {
        sqlx::query_with(
            &LoanApplications::build_insert_string(LoanApplications::TABLE, &LoanApplications::COLUMNS_ARRAY),
            entity.get_args(),
        )
        .execute(&connection)
        .await
    ...
    async fn update(connection: PgPool, entity: &LoanApplications) -> Result<bool, Box<dyn StdError>> {
        sqlx::query_with(
            &LoanApplications::build_update_string(LoanApplications::TABLE, &LoanApplications::COLUMNS_ARRAY, LoanApplications::PK),
            entity.get_args(),
        )
        .execute(&connection)
        .await
    ...
    async fn delete(connection: PgPool, id: &String) -> Result<bool, Box<dyn StdError>> {
        ...
        sqlx::query_with(
            &LoanApplications::build_delete_string(LoanApplications::TABLE, LoanApplications::PK),
            args,
        )
        .execute(&connection)
        .await
