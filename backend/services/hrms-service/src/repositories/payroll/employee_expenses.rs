...
impl IRepository<EmployeeExpenses> for EmployeeExpensesRepository {
    async fn get_all(connection: PgPool) -> Result<Vec<EmployeeExpenses>, Box<dyn StdError>> {
        let result = sqlx::query(format!("SELECT {} FROM {}", EmployeeExpenses::COLUMNS, EmployeeExpenses::TABLE).as_str())
...
let _ = args.add(entity.status_id.clone());
        ...
        sqlx::query_with(
            format!("UPDATE {} SET {}", EmployeeExpenses::TABLE, EmployeeExpenses::COLUMNS_UPDATE).as_str(),
            args,
        )
        .execute(&connection)
        .await
    ...
    async fn delete(connection: PgPool, id: &String) -> Result<bool, Box<dyn StdError>> {
        ...
        sqlx::query_with(
            format!("DELETE FROM {} WHERE {}", EmployeeExpenses::TABLE, EmployeeExpenses::PK).as_str(),
            args,
        )
        .execute(&connection)
        .await
