...
impl IRepository<SaleOrders> for SaleOrdersRepository {
    async fn get_all(connection: PgPool) -> Result<Vec<SaleOrders>, Box<dyn StdError>> {
        let result = sqlx::query(&SaleOrders::build_select_string(
            SaleOrders::TABLE,
            &SaleOrders::COLUMNS_ARRAY,
            None,
        ))
    ...
    async fn add(connection: PgPool, entity: &SaleOrders) -> Result<bool, Box<dyn StdError>> {
        sqlx::query_with(
            &SaleOrders::build_insert_string(SaleOrders::TABLE, &SaleOrders::COLUMNS_ARRAY),
            entity.get_args(),
        )
        .execute(&connection)
        .await
    ...
    async fn update(connection: PgPool, entity: &SaleOrders) -> Result<bool, Box<dyn StdError>> {
        sqlx::query_with(
            &SaleOrders::build_update_string(SaleOrders::TABLE, &SaleOrders::COLUMNS_ARRAY, SaleOrders::PK),
            entity.get_args(),
        )
        .execute(&connection)
        .await
    ...
    async fn delete(connection: PgPool, id: &String) -> Result<bool, Box<dyn StdError>> {
        ...
        sqlx::query_with(
            &SaleOrders::build_delete_string(SaleOrders::TABLE, SaleOrders::PK),
            args,
        )
        .execute(&connection)
        .await
